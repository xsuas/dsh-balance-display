/**
 * DeepSeek API balance host plugin.
 */
...
export function apply(ctx) {
  ...
  // Resolve credentials for every request.
  async function fetchFresh() {
    ...
    try {
      ...
      if (error?.name === "TimeoutError" || error?.name === "AbortError") {
        throw fail("upstream-timeout", "DeepSeek 余额接口请求超时");
      }
      throw fail("upstream-network", "无法连接 DeepSeek 余额接口");
    }
    if (!response.ok) {
  ...
  // Reuse cached or in-flight balance requests.
  ...
  // Remove the route when the plugin unloads.
