...
| `plugins/client-balance-display` | `@xsuas/dsh-client-balance-display` | 客户端插件：输入框工具行左侧"余额"芯片，直接显示 DeepSeek API 账户余额（点击手动刷新、10 分钟自动重拉）。会话 token 用量由官方统计行原生展示，不重复实现 |
